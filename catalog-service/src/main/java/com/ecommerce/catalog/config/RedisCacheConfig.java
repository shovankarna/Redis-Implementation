...
public class RedisCacheConfig {
    ...
    public RedisCacheManager cacheManager(RedisConnectionFactory connectionFactory) {
        ...
        // 1. Your default config (e.g., 60 minutes)
        RedisCacheConfiguration defaultCacheConfig = RedisCacheConfiguration.defaultCacheConfig()
                .serializeValuesWith(RedisSerializationContext.SerializationPair
                        .fromSerializer(new GenericJackson2JsonRedisSerializer()))
                .entryTtl(Duration.ofMinutes(defaultTtlMinutes));

        // 2. Create a CUSTOM config with an extended TTL (e.g., 24 hours)
        ...
        return RedisCacheManager.builder(connectionFactory)
                ...
                .withCacheConfiguration("products", defaultCacheConfig)
                .withCacheConfiguration("categories", longLivedConfig) // Extended cache ttl where required
                .enableStatistics()
                .build();
